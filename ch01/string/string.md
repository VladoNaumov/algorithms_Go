...
> Если одно слово можно получить, просто переставив буквы другого слова — это **анаграмма**, или **кодовое слово**.

### Примеры
```
| Слово 1      | Слово 2       | Результат      | Пояснение                                               |
| ------------ | ------------- | -------------- | ------------------------------------------------------- |
| `listen`     | `silent`      | ✅ анаграммы    | одинаковый набор букв                                   |
| `Astronomer` | `Moon starer` | ✅ анаграммы    | после удаления пробелов и приведения к нижнему регистру |
| `Hello`      | `Olelh`       | ✅ анаграммы    | просто перестановка                                     |
| `кот`        | `ток`         | ✅ анаграммы    | русская версия                                          |
| `code`       | `decode`      | ❌ не анаграммы | буквы не совпадают по количеству                        |
```
**Сложность:**
...
Хочешь, я теперь оформлю для этих 16–25 задач **мини-таблицу “Что тренирует”** (например: работа со стеком, итерацией, частотным анализом, регулярками и т.д.) — чтобы у тебя получился учебный конспект “по темам”?
